...
export async function getGoogleReverseGeocode(latitude, longitude) {
  const apiKey = process.env.GOOGLE_MAPS_SERVER_API_KEY;
  if (!apiKey) throw new Error("GOOGLE_MAPS_NOT_CONFIGURED");
  const response = await fetch(
    `https://maps.googleapis.com/maps/api/geocode/json?latlng=${encodeURIComponent(latitude)},${encodeURIComponent(longitude)}&language=fr&key=${apiKey}`,
    { cache: "no-store" }
  );
  if (!response.ok) throw new Error("GOOGLE_MAPS_REQUEST_FAILED");
  const result = (await response.json()).results?.[0];
  if (!result) throw new Error("ROUTE_NOT_FOUND");
  const cityComponent = result.address_components?.find((part) => part.types?.includes("locality"))
    || result.address_components?.find((part) => part.types?.includes("postal_town"));
  return {
